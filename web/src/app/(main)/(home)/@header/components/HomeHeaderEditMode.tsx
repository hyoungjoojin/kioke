...
import { useUpdateDashboardMutationQuery } from '@/query/dashboard/updateDashboard';
...
export default function HomeHeaderEditMode() {
  const t = useTranslations();

  const { dashboardDraft } = useDashboard();
  const { updateDashboardDraft, setIsEditingDraft } = useDashboardActions();
  const { mutate: updateDashboard, isPending: isUpdateDashboardPending } =
    useUpdateDashboardMutationQuery({
      onSuccess: () => {
