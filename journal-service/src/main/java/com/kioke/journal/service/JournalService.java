...
// String title = createJournalDto.getTitle(), template =
// createJournalDto.getTemplate();

// Journal journalToSave =
// Journal.builder().title(title).template(template).pages(new
// ArrayList<Page>()).build();

// Journal savedJournal = journalRepository.save(journalToSave);

// try {
// String authServiceUri =
// discoveryClientService.getServiceUri(KiokeServices.AUTH_SERVICE);
...
// try {
// Journal journal =
// journalRepository.findById(jid).orElseThrow(() -> new
// JournalNotFoundException(jid));
